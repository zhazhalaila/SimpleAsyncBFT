...
def start_server(n, f, id):
    if id >= 10:
        command = "go run -race main.go -path=log{}.txt -n={} -f={} -id={} -port=:80{}".format(id, n, f, id, id)
    else:
        command = "go run -race main.go -path=log{}.txt -n={} -f={} -id={} -port=:800{}".format(id, n, f, id, id)
